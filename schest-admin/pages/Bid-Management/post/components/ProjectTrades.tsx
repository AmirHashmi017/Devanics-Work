...
import { isArrayString } from 'src/utils/typescript.utils';
...
export function PostProjectTrades({ formik, children }: Props) {
  ...
  function deSelectSubCategories(parentId: string) {
    ...
    // @ts-ignore
    const updatedTrades = formik.values.selectedTrades.filter(
      (trade: string | ITrade) => {
    ...
    formik.setFieldValue('selectedTrades', updatedTrades);
