...
import { Skeleton, Spin, UploadFile } from 'antd';
...
export function ProjectIntro({ id }: Props) {
  ...
  const query = useQuery(
    ...
    () => {
      return bidManagementService.httpGetProjectById(id);
    },
    {
      onSuccess(data) {
    ...
    <div className="flex justify-between items-center">
      <div className="space-y-3">
        ...
        <div className="flex space-x-4 items-center text-[#667085] text-base leading-6 font-normal">
          <SenaryHeading
            title={`Creation Date: ${
              bid
                ? momentTimezone(bid.createdAt).format('MM/DD/YYYY hh:mm A')
                : ''
            }`}
          />
          <SenaryHeading
            title={`Bid Date: ${
              bid
                ? momentTimezone(bid.bidDueDate).format('MM/DD/YYYY hh:mm A') +
                  ' ' +
      ...
      <div className="flex items-center space-x-3 flex-1 justify-end">
              ...
              >
                {['active', 'archived'].map((status) => {
                  return (
                    <p
                      key={status}
                      className={`text-[#344054] hover:bg-gray-50 px-2 py-1 cursor-pointer rounded-lg  font-normal capitalize text-medium leading-6 ${
                        bid && bid.status === status ? 'bg-gray-100' : ''
                      }`}
                      onClick={() => {
        ...
        <div className="w-fit">
          ...
          />
          <ModalComponent open={showUpdateModal} setOpen={setShowUpdateModal}>
            <div>
              <Popups
                title="Update"
                onClose={() => {
