...
import { IExpiredUserResponse, userService } from 'src/services/user.service';
...
type DataType = IExpiredUserResponse['users'][0];
...
export const UserManagementExpiredTable = forwardRef<
  { handleExport: () => void },
  Props
>(({ role, search, selectedRowKeys, setSelectedRowKeys }, ref) => {
  ...
  const [data, setData] = useState<IExpiredUserResponse>({
    count: 0,
    users: [],
  });
  ...
  const columns: ColumnsType<DataType> = [
...

    {
      ...
      render(value, record, index) {
        if (record && typeof record.planId !== 'string') {
          return <div className="capitalize">{record.planId.planName}</div>;
        }
        return null;
      },
    },

    {
      ...
      render(value, record, index) {
        if (record && typeof record.planId !== 'string') {
          return <div className="capitalize">{record.planId.type}</div>;
        }
        return null;
      },
    },
    {
      ...
      render(value: DataType['subscription']) {
        if (value && value.status === 'expired') {
          return moment(value.expiredAt).format('DD/MM/YYYY');
        }
        return null;
      },
    },
  ...
  async function getUsers() {
    setLoading(true);
    try {
      const response = await userService.httpGetExpiredUsers({
        ...pagination,
        role,
      });
      if (response.data) {
  ...
  useImperativeHandle(ref, () => ({
    handleExport: () =>
      exportToCSV(data.users, roleBasedColumns, 'Expired Users'),
  }));
