...
export const UserManagementInvitedTable = forwardRef<
  {
    handleExport: () => void;
  },
  Props
>(({ role, search, selectedRowKeys, setSelectedRowKeys }, ref) => {
  ...
  async function getUsers() {
    setLoading(true);
    try {
      const response = await userService.httpGetInvitedUsers({
        ...pagination,
        role,
      });
      if (response.data) {
  ...
  const columns: ColumnsType<DataType> = [
...

    {
      ...
      render(value, record, index) {
        if (!record || !record.invitation) return null;
        return moment(record.invitation.date).format('DD-MM-YYYY');
      },
    },
  ...
  useImperativeHandle(ref, () => ({
    handleExport: () =>
      exportToCSV(data.users, roleBasedColumns, 'Invited Users'),
  }));
