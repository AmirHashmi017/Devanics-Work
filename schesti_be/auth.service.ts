...
class AuthService {
  ...
  const { _id } = req.payload;
  const { planID, autoRenew, countryCode } = req.body;
  ...
  // 3. Get pricing based on country
  const pricing = this.getPricingForUser(selectedPlan, countryCode);

  // 4. Validate Stripe support
  if (!pricing.stripeSupported) {
    throw new CustomError(
      EHttpStatus.BAD_REQUEST,
      `Stripe doesn't support ${pricing.currency}. Please use Paymob instead.`
    );
  }

  // 5. Get correct Stripe price ID
  const stripePriceId = pricing.isRegional && pricing.stripePriceId
  ...
  try {
    const session = await StripeHelper.checkout.sessions.create({
      ...
      customer_email: userDetail.stripeCustomerId ? undefined : userDetail.email,
      customer: userDetail.stripeCustomerId || undefined,
      metadata: {
        planId: planID,
        customerId: _id,
        autoRenew: autoRenew,
  ...
  async stripeSuccessRequest(req) {
    ...
    switch (event.type) {
      ...
      case 'checkout.session.completed':
        ...
        const paidPrice = Number(metadata.pricePaid || selectedPlan.basePrice);
        const paidCurrency = (metadata.currency || 'USD').toUpperCase();
 ...
 private getPricingForUser(plan: any, userCountry: string) {
  const regionalPrice = plan.localizedPricing?.find(
    (rp) => rp.countryCode?.toUpperCase() === userCountry?.toUpperCase()
  );
  ...
  if (regionalPrice) {
    return {
      ...
      isRegional: true,
      stripeSupported: regionalPrice.stripeSupported,
      paymobSupported: regionalPrice.paymobSupported,
      stripePriceId: regionalPrice.stripePriceId,
      paymobPlanId: regionalPrice.paymobPlanId,
    };
  }

  // Fallback to USD base price
  return {
    price: plan.basePrice,
    currency: plan.baseCurrency || 'USD',
    isRegional: false,
    stripeSupported: true, // USD is always supported by Stripe
  ...
  const { planID, autoRenew, promoPrice, countryCode } = req.body;
  ...
  const pricing = this.getPricingForUser(plan, countryCode);
  if (!pricing.paymobSupported) {
    return {
      statusCode: EHttpStatus.BAD_REQUEST,
      message: `Paymob doesn't support ${pricing.currency}. Please use Stripe instead.`,
    };
  }

  if (!pricing.paymobPlanId) {
    return {
      statusCode: EHttpStatus.BAD_REQUEST,
      message: "This plan doesn't have Paymob configuration for your currency",
    };
  }

  // Use promoPrice if provided and valid
  const finalPrice = promoPrice && !isNaN(Number(promoPrice)) && Number(promoPrice) > 0
    ? Number(promoPrice)
    : pricing.price;
  ...
  try {
    const response = await Paymob.createPaymentIntention({
      ...
      extras: {
        ...
        currency: pricing.currency,
        promoApplied: promoPrice && Number(promoPrice) < pricing.price,
      },
      items: [
        {
          amount: amountInCents,
      ...
      subscription_plan_id: pricing.paymobPlanId,
      ends_at,
    });
