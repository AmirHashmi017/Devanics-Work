...
const EditableArcShape: React.FC<EditableCurvedShapeProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      cur?.controlPoints &&
      Array.isArray(cur?.controlPoints) &&
      cur?.controlPoints?.length > 0
    ) {
  ...
  console.log(cur, controlPoints, points, ' ===> points of 1drag');
          ...
          handleMsrOprations('update', {
            ...
            data: {
              ...
              text: calculateLineDistance(points, controlPoints)
                ? cur?.scalUnits == 'feet'
                  ? calculateLineDistance(points, controlPoints, true)
                  : `${Number(Number(calculateLineDistance(points, controlPoints, false)) * 0.0254).toFixed(3)} meter`
                : cur?.text,
            },
          });
  ...
  return (
    <>
      <Line
        ...
        stroke={
          selectedShape?.split('-')[2] === rest?._id ? 'maroon' : cur?.stroke
        }
        dash={selectedShape?.split('-')[2] === rest?._id ? [10, 10] : []}
      ...
      <Text
        ...
        // text={dis + ''}
        text={`${
          // calculateLineDistance(points, controlPoints)
          cur?.scalUnits == 'feet'
            ? calculateLineDistance(points, controlPoints, true)
            : `${Number(Number(calculateLineDistance(points, controlPoints, false)) * 0.0254).toFixed(3)} meter`
        }`}
