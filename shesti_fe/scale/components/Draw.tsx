...
interface Props {
  ...
  handleMsrOprations: (type: 'create' | 'update' | 'delete', data: any) => void;
}
...
const Draw: React.FC<Props> = ({
  ...
  const handleMouseDown = (e: KonvaEventObject<MouseEvent>) => {
    ...
    if (selected === 'length' && currentLine.startingPoint) {
      ...
      handleMsrOprations('create', { ...newLine, type: 'line' });
    ...
    if (selected === 'arc' && currentLine.startingPoint) {
      ...
      const newLine: LineInterface = {
        ...
        textUnit: unit,
        dateTime: moment().toDate(),
        projectName: 'Arc LengthMeasurement',
        category: selectedCategory ?? 'Arc Length Measurement', //(selectedCategory && selectedCategory?.length > 0) ? selectedCategory : 'Length Measurement',
        subcategory: selectedSubCategory,
        user,
        textColor: textColor,
        text: lineDistance?.toString(),
      ...
      handleMsrOprations('create', { ...newLine, type: 'arc' });
    ...
    if (selected === 'rectangle' && currentLine.startingPoint) {
      ...
      handleMsrOprations('create', { ...areaConfig, type: 'area' });
          ...
          if (
            ...
            setDraw((prevDraw: any) => {
              ...
              if (selected === 'area') {
                ...
                const text = `${volume?.toFixed(2) || ''} CF`;
          ...
          if (
            ...
            setDraw((prevDraw: any) => {
              ...
              const areaConfig: PolygonConfigInterface = {
                ...
                textUnit: unit,
                dateTime: moment().toDate(),
                projectName: 'Curve Area Measurement',
                category: selectedCategory ?? 'Curve Area Measurement', //(selectedCategory && selectedCategory?.length > 0) ? selectedCategory : 'Length Measurement',
                subcategory: selectedSubCategory,
                user,
                textColor: textColor,
                fillColor: fillColor,
              ...
              updateDrawHistory(pageNumber.toString(), 'area', areaConfig);
              handleMsrOprations('create', { ...areaConfig, type: 'curve' });
    ...
    if (selected === 'count' && position) {
      ...
      handleMsrOprations('create', { ...newCount, type: 'count' });
  ...
  const handleStageClick = (e: Konva.KonvaEventObject<MouseEvent>) => {
    ...
    if (stage) {
      ...
      if (mousePos) {
        ...
        handleMsrOprations('create', { ...newText, type: 'texts' });
      }
    }
    // }
  };
  const handleDelete = (text: any) => {
    console.log(text, ' ===> handle text edit props');
    // alert(`${text?.id}`)
    if (text?.idn) {
      const tx = texts.filter((i: any) => i?.id != text?.idn);
      const tx1 = draw.texts.filter((i: any) => i?.id != text?.idn);
      setTexts(tx);
      setDraw((ps: any) => ({ ...ps, texts: tx1 }));
      if (text?.cur?._id) {
        handleMsrOprations('delete', { id: text?.cur?._id });
      }
    }
  };

  const handleTextChange = (id: string, newTextProps: any, db_id: string) => {
    console.log(id, newTextProps, db_id, ' ===> handle text edit props');
    // alert(JSON.stringify(newTextProps))
    setTexts(
      texts.map((text) =>
        text.idn === id ? { ...text, ...newTextProps } : text
      )
    );
    setDraw((ps: any) => ({
      ...ps,
    ...
    if (db_id) {
      handleMsrOprations('update', { id: db_id, data: { ...newTextProps } });
    }
  };
  ...
  return (
    <div
      ref={stageParentRef}
      id="sage-parent"
      className={`outline-none relative bg-grey-900 overflow-auto !w-full !h-full`}
      tabIndex={1}
      onKeyDown={(e) => {
        if (e.key === 'Escape') {
          setCurrentLine(defaultCurrentLineState);
        ...
        if (e.key === 'Delete' && selectedShape) {
          const [shapeName, shapeNumber, _id] = selectedShape.split('-');
          ...
          setDraw((prev: any) => {
            ...
            handleMsrOprations('delete', { id: _id });
      ...
      >
        ...
        >
                ...
                >
                  <EditableText
                    ...
                    onChange={(newTextProps) =>
                      handleTextChange(text?.idn, newTextProps, text?._id)
                    }
                    textColor={text?.textColor ?? textColor}
                    handleDelete={handleDelete}
                    selectedCategory={selectedCategory}
                    selectedSubCategory={selectedSubCategory}
                    ctrlPressed={selectedShape?.split('-')[2] === text?._id}
                    cur={text}
            ...
            return (
              <Group
                ...
                onClick={(e) => {
                  e.cancelBubble = true;
                  setSelectedShape(
                    e.currentTarget.attrs?.id + `-${rest?._id}` || ''
                  );
                }}
              >
                <Arrow
                  ...
                  lineCap="round"
                  dash={
                    selectedShape?.split('-')[2] === rest?._id ? [10, 10] : []
                  }
                  stroke={
                    selectedShape?.split('-')[2] === rest?._id
                      ? 'maroon'
                      : rest?.stroke
                  }
                  pointerAtEnding={true}
                  ...
                  onDragEnd={(e) => {
                    const [shapeName, shapeNumber, _id] = id.split('-');
                    console.log(shapeNumber, shapeName, _id);
                    const node = e.target as any;
            ...
            return (
              <Group
                ...
                onClick={(e) => {
                  e.cancelBubble = true;
                  setSelectedShape(
                    e.currentTarget.attrs?.id + `-${cur?._id}` || ''
                  );
                }}
              >
                {/* <Arrow
          ...
          {draw?.area?.map(({ textUnit, ...rest }: any, index: number) => {
            ...
            const text =
              rest?.scalUnits == 'feet'
                ? `${roundDecimals(area, scale?.precision) || ''}SF`
          ...
          {draw?.volume?.map(
            ({ depth, textUnit, ...rest }: any, index: number) => {
              ...
              const text =
                rest?.scalUnits == 'feet'
                  ? `${volume?.toFixed(2) || ''} CF`
                  : `${Number(volume * 0.0283168).toFixed(3)} m³`;
