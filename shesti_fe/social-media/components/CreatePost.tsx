...
export function CreatePost({ onPostAction }: CreatePostProps) {
  ...
  return (
    <div className="w-full max-w-3xl bg-white rounded-3xl shadow-sm border border-gray-200 p-6">
      <ModalComponent open={isModalOpen} setOpen={() => setIsModalOpen(false)}>
        <Popups
          title={postData ? 'Edit Post' : 'Create Post'}
          onClose={() => {
...
export function CreatePostModal({
  ...
  async function createPost() {
    ...
    try {
      ...
      if (mediaFilesLength || description) {
        ...
        onPostAction?.();
        // FIX 1: Trigger feed refresh immediately after post creation
  ...
  async function updatePost() {
    try {
      ...
      if (mediaFilesLength || postOldMediaUrls.length || description) {
        ...
        toast.success(message);
        onPostAction?.();
        // FIX 1: Trigger feed refresh immediately after post update
        dispatch(setFetchPosts());
        dispatch(setPostData(null));
        onClose();
        resetPost();
  ...
  return (
    <div>
      ...
      <div className="px-4 py-3 flex justify-start gap-4">
      ...
      <div className="p-4 flex gap-3">
        <CustomButton
          text={postData ? 'Save' : 'Create Post'}
          className="flex-1"
