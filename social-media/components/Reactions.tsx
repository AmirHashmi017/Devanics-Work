...
type Props = {
  ...
  isPost?: boolean;
  onReactionUpdate?: (reaction: IUserReaction | null) => void; // Add callback prop
};
  ...
  const addPostReactionHandler = async ({
    ...
    try {
      ...
      if (isPost) {
        ...
        updatedReaction = removeReaction ? null : response.data.reaction; // Adjust based on API response
      } else {
      ...
      if (removeReaction) {
        ...
        setCurrentReactions((prev) =>
          prev.filter(({ associatedCompany }) => associatedCompany !== user?._id)
        );
      } else {
        ...
        setCurrentReactions((prev) => {
          ...
          if (loginUserReactionData) {
            updatedReactions = prev.map((reactionData) =>
              reactionData.associatedCompany === user?._id
                ? { ...reactionData, type }
                : reactionData
            );
          } else {
            updatedReactions = [
              ...prev,
              {
                ...
                type,
                _id: new Date().getTime().toString(), // Temporary ID, replace with API response if available
              },
            ];
          }
